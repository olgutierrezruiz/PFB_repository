...
#  This script should be executed from correction/ecoli-80.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./ecoli-80.ovlStore.BUILDING/$jobname.info -a ! -e ./ecoli-80.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './ecoli-80.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./ecoli-80.ovlStore.BUILDING \
  -S ../ecoli-80.seqStore \
  -C  ./ecoli-80.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
