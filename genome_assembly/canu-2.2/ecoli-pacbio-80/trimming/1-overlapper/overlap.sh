...
if [ $jobid -eq 1 ] ; then
  bat="001"
  job="001/000001"
  opt="-h 1-9146 -r 1-9146 --hashdatalen 80013724"
fi

if [ $jobid -eq 2 ] ; then
  bat="001"
  job="001/000002"
  opt="-h 9147-10043 -r 1-10043 --hashdatalen 8233657"
fi
...
if [ ! -e ../0-mercounts/ecoli-80.ms22.dump ] ; then
  mkdir -p ../0-mercounts
  cd ../0-mercounts
  cd -
fi
...
$bin/overlapInCore \
  ...
  -t 8 \
  -k 22 \
  -k ../0-mercounts/ecoli-80.ms22.dump \
  --hashbits 22 \
  --hashload 0.8 \
  --maxerate  0.045 \
  --minlength 500 \
