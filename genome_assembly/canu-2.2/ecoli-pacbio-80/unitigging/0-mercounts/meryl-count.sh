...
if [ -e ./ecoli-80.$jobid.meryl/merylIndex ] ; then
...
if [ -e ecoli-80.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/Users/student/genome_assembly/canu-2.2/bin/meryl k=22 threads=4 memory=2 \
  count \
    segment=$jobid/01 ../../ecoli-80.seqStore \
    output ./ecoli-80.$jobid.meryl.WORKING \
&& \
mv -f ./ecoli-80.$jobid.meryl.WORKING ./ecoli-80.$jobid.meryl
